...
["source", "declarative"].forEach((listType) => {
  ...
  describe(`A ${listType} List`, () => {
    ...
    describe("should set aria attributes on list items", () => {
      it("should set aria-checked for selected item", () => {
        ...
        cy.get("#list-item-0").should("not.have.attr", "aria-checked");
        cy.get("#list-item-1").should("have.attr", "aria-checked");
      });
    ...
    describe("when multi-selection is enabled", () => {
      it("should set aria-multiselectable on the list", () => {
        cy.mount(
          isDeclarative ? (
            ...
            <List selectionVariant="multiple" source={ITEMS} />
          )
        );
      ...
      it("should set aria-selected for selected items", () => {
        cy.mount(
          isDeclarative ? (
            <List id="list" selectionVariant="multiple">
              <ListItem>list item 1</ListItem>
              <ListItem>list item 2</ListItem>
            </List>
          ) : (
            <List id="list" selectionVariant="multiple" source={ITEMS} />
          )
        );
