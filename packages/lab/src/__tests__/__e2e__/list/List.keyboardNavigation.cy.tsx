...
["source", "declarative"].forEach((listType) => {
  ...
  describe(`A ${listType} supports type to select`, () => {
    ...
    beforeEach(() => {
      onChange = cy.stub().as("changeHandler");
      const listProps = { id: "list", onChange };
    ...
    it("supports item selection using the Spacebar after search times out", () => {
      ...
      cy.get("#list-item-2").should("not.have.attr", "aria-checked", "true");
      // Verify onChange was not called
      cy.get("@changeHandler").should("not.have.been.called");

      cy.wait(1500);
      cy.realPress("Space");
      cy.get("#list-item-2").should("have.attr", "aria-checked", "true");
      cy.get("@changeHandler").should(
        "have.been.calledWith",
        Cypress.sinon.match.any,
        "Foo Bar"
  ...
  it(`A ${listType} can disable type to select`, () => {
    cy.mount(
      isDeclarative ? (
        <List disableTypeToSelect displayedItemCount={ITEMS_PER_PAGE} id="list">
          {FancyItems.map((x, i) => (
            <ListItem key={`item-${i}`}>{x}</ListItem>
          ))}
        </List>
...
["source", "declarative"].forEach((listType) => {
  describe(`A ${listType} List supports tab to select when turned on`, () => {
    ...
    it("should select the highlighted item", () => {
      cy.findByRole("listbox").focus();
      cy.realPress("Tab");
      cy.get("#list-item-0").should("have.class", "uitkListItem-selected");
    });
  });
});

describe("GIVEN a grouped List", () => {});
