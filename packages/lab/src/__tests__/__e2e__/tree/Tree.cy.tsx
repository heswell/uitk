...
import { TreeNext } from "@jpmorganchase/uitk-lab";
...
describe("GIVEN a Tree", () => {
  describe("AND when the component renders with empty root childNodes", () => {
    beforeEach(() => {
      cy.mount(
        <TreeNext source={[{ id: "a", label: "name", childNodes: [] }]} />
      );
    });
  ...
  describe("AND when the component renders with `null` root childNodes", () => {
    beforeEach(() => {
      cy.mount(
        <TreeNext source={[{ id: "a", name: "name", childNodes: null }]} />
      );
    });
  ...
  describe('AND when `selectionStrategy` is set to "default"', () => {
    it("THEN should select 1 node", () => {
      cy.mount(
        <TreeNext source={createSampleTreeData()} selectionStrategy="default" />
      );
  ...
  describe("AND some nodes are pre-selected", () => {
    beforeEach(() => {
      ...
      cy.mount(<TreeNext source={source} selectionStrategy="default" />);
    });
