...
const pageDown = (
  ...
  let { top: itemTop } = itemEl.getBoundingClientRect();
  ...
  if (newScrollTop !== scrollTop && index < lastIndexPosition) {
    containerEl.scrollTo(0, newScrollTop + 1);
...
>({
  ...
  const nextFocusableItemIdx = useCallback(
    (
      ...
      const startIdx =
        key === End ? indexPositions.length : key === Home ? -1 : idx;
    ...
    (e: FocusEvent) => {
      ...
      if (ignoreFocus.current) {
        ...
        if (highlightedIndex !== -1) {
          ...
          setHighlightedIndex(lastFocus.current === -1 ? 0 : lastFocus.current);
        } else if (hasSelection(selected)) {
          const firstSelectedItem = getFirstSelectedItem(selected);
          if (firstSelectedItem) {
            const idx = indexPositions.indexOf(firstSelectedItem);
  ...
  const listProps = useMemo(() => {
    return {
      onBlur: (e: FocusEvent) => {
        const sourceTarget = (e.target as HTMLElement).closest(".uitkListNext");
