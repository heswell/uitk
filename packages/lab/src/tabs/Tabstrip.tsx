...
import { ListNextProps /*, ListSingleSelectionVariant */ } from "../list-next";
import { Tooltip, useTooltip } from "../tooltip";
import { DropdownList } from "../dropdown-next";
...
export const Tabstrip = forwardRef(function Tabstrip(
  ...
  const activeRef = useRef(activeTabIndexProp || defaultActiveTabIndex || 0);
  ...
  const tabstripHook = useTabstrip({
    activeTabIndex: activeTabIndexProp,
    allowDragDrop,
    collectionHook,
  ...
  activeRef.current = tabstripHook.activeTabIndex;
  ...
  const handleAddTabClick = useCallback(() => {
    ...
    onAddTab?.();
  }, [collectionHook.isControlled, onAddTab, tabstripHook.addTab]);

  const selectedIndex = useRef(tabstripHook.activeTabIndex ?? 0);
  const focusedTabIndex = tabstripHook.highlightedIdx;
  const handleOverflowSelectionChange: ListNextProps<OverflowItem>["onSelectionChange"] =
    useCallback(
      (e, tab) => {
        if (tab !== null) {
          tabstripHook.activateTab(tab.index);
        }
      },
      [tabstripHook.activateTab]
    );
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (
      selectedIndex.current !== tabstripHook.activeTabIndex &&
      overflowMenuProp
    ) {
  ...
  }, [overflowMenuProp, tabstripHook.activeTabIndex]);
  ...
  useIsomorphicLayoutEffect(() => {
    if (
      focusedTabIndex !== tabstripHook.activeTabIndex &&
      focusedTabIndex !== -1
    ) {
      tabstripHook.focusTab(tabstripHook.activeTabIndex);
    }
  ...
  }, [tabstripHook.activeTabIndex]);

  // useLayoutEffectSkipFirst(() => {
  //   collectionHook.dispatch({ type: "reset" });
  // }, [collectionHook.dispatch]);

  const renderContent = () => {
    const content = collectionHook.data
      .filter((item) => !item.isOverflowIndicator && !item.isInjectedItem)
      .map((item: OverflowItem<any>, index: number) => {
        const tab = item.source as TabDescriptor;
        const element = item.element as TabElement;

        const selected = index === tabstripHook.activeTabIndex;
    ...
    const draggingActiveTab =
      tabstripHook.draggedItemIndex === tabstripHook.activeTabIndex;
    const showOverflow =
  ...
  const selectedTabOverflowed = overflowedItems.some(
    (item: OverflowItem) => item.index === tabstripHook.activeTabIndex
  );
  const className = cx(
    withBaseName(),
  ...
  const { id: selectedTabId } =
    collectionHook.data[tabstripHook.activeTabIndex];
