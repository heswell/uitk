...
import { createDropIndicatorPosition as createDragDisplacer } from "./Draggable";
  ...
  const animateTransition = useCallback(
    (size: number, propertyName = "width") => {
      ...
      animationFrame.current = requestAnimationFrame(() => {
        ...
        spacer1.style.cssText = `${propertyName}: ${SPACER_SIZE}px`;
        spacer2.style.cssText = `${propertyName}: ${SPACER_SIZE}px`;
        spacers[0] = spacer2;
        spacers[1] = spacer1;
      });
    },
    ...
    (
      ...
      if (dropTarget) {
        ...
        if (useTransition) {
          ...
          spacer1.style.cssText = `${propertyName}: ${SPACER_SIZE}px`;
          console.log(
            `add dropTargetPosition before ${dropTarget.element.textContent}`
          );
          dropTarget.element.before(spacer1);
          return spacer1;
        } else {
