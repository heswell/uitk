...
- d460fb7b: Changed `--salt-size-detail` to `--salt-size-compact`
- 5ead0bbf: Renamed `--salt-size-selection` to `--salt-size-selectable`
...
- d975171f: Changed `--salt-size-brandBar` to `--salt-size-accent` with new density dependent values
- 9cae606a: Removed differential characteristic
