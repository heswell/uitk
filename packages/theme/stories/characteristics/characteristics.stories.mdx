...
import { Card, Input, Panel, FormField } from "@heswell/uitk-core";
...
Alternatively, if you want it to have a lower emphasis, pass in the `uitkEmphasisLow` className to FormField, and a transparent background displays:
...
Not all UITK components support emphasis by default. This capability can easily be added though.
In the following example, we will show how to add emphasis to the Card component. Currently, Card makes use of the `---uitk-container-background-medium` token for all of it's styling purposes. Here's how it looks by default:
...
Note the use of `-high` emphasis characteristic tokens provided by UITK that weren't used in Card by default, as well as raw values to further emphasize the component. Now, all we have to do to make Card have emphasis is pass in the `uitkEmphasisHigh` class name:
